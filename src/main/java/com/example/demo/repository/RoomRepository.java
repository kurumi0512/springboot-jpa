...
public interface RoomRepository extends JpaRepository<Room, Integer> { // Room: entity, Integer: @Id 的型別
	...
	// 如果超過三個以上的Than ...between ...and就要用另外一種方式
	List<Room> findByRoomSizeGreaterThan(Integer size);

	// 2. 查詢 roomSize 大於指定值得房間(自行撰寫 T-SQL, 注意:欄位名要符合資料表中的設定,findRooms可以自行自訂名字)
	...
	// 3. 查詢 roomSize 大於指定值得房間(自行撰寫 PQL, 注意:欄位名要符合 entity 中的設定)
